...
def feature_extraction(series, vec_args):
    ...
    vectorizer1 = TfidfVectorizer(max_df=vec_args["max_df"], min_df=vec_args["min_df"], max_features=vec_args["max_features"])
...
def draw_clustering_analysis_pie(rank_num, value, yticks, title, t):
    ...
    plt.title(title)
    plt.savefig('my_img/' + str(t) + '/1.jpg')
...
def fun(ttt):
    ...
    plt.savefig('my_img/' + str(ttt) + '/聚类效果.jpg')
    plt.show()
